...
def index(request):
    if request.method == 'POST':
        ...
        if formats:
            return render(request, 'instadownloader/formats.html', {'formats': formats, 'url': url})
        else:
...
def download_view(request):
    if request.method == 'POST':
        ...
        success, filename = download_instagram_video(url, format_id, output_dir)
        if success:
            video_path = os.path.join(settings.MEDIA_URL, filename)
            return render(request, 'instadownloader/download.html', {'video_path': video_path})
        else:
